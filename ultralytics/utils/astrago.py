...
class Astrago(tqdm):
    ...
    def format_meter(n, total, elapsed, rate=None, initial=0, *args, **kwargs):
        ...
        gpu_usage = (
            sum(Astrago.gpu_memory_usage) / (len(Astrago.gpu_memory_usage) - 1)
            if n != 0
            else sum(Astrago.gpu_memory_usage) / len(Astrago.gpu_memory_usage)
        )
        cpu_usage = (
            sum(Astrago.cpu_usage) / (len(Astrago.cpu_usage) - 1)
            if n != 0
            else sum(Astrago.cpu_usage) / len(Astrago.cpu_usage)
        )
        ...
        if n == total:
            if (
                not hasattr(Astrago, "last_epoch_done") or not Astrago.last_epoch_done
            ):  # 마지막 에폭이 한 번만 출력되도록 확인하는 변수
                Astrago.save_metrics_to_csv(
                    Astrago.model_name,
                    Astrago.param,
                    Astrago.gpu,
                    Astrago.FLOPS,
...
class Inference(tqdm):
    ...
    def format_meter(n, total, elapsed, rate=None, initial=0, *args, **kwargs):
        torch.cuda.synchronize()
        gpu_usage = (
            sum(Inference.gpu_memory_usage) / (len(Inference.gpu_memory_usage) - 1)
            if n != 0
            else sum(Inference.gpu_memory_usage) / len(Inference.gpu_memory_usage)
        )
        cpu_usage = (
            sum(Inference.cpu_usage) / (len(Inference.cpu_usage) - 1)
            if n != 0
            else sum(Inference.cpu_usage) / len(Inference.cpu_usage)
        )
        ...
        if total != 1:
            ...
            if n == total:
                if (
                    not hasattr(Inference, "last_epoch_done") or not Inference.last_epoch_done
                ):  # 마지막 에폭이 한 번만 출력되도록 확인하는 변수
                    Inference.save_metrics_to_csv(
