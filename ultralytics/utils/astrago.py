...
class Astrago(tqdm):
    ...
    # csv 파일 저장 함수
    def save_metrics_to_csv(model_name, param, gpu, FLOPS, class_num, t_img_num, t_instance_num, v_img_num, v_instance_num,
                            imgsz, batch, epoch, preprocess_time, train_time, val_time, save_model_time, scheduler_time, epoch_time, 
        ...
        with open(Astrago.csv_file_path, mode='a', newline='', encoding='utf-8') as file:
            ...
            if file.tell() == 0:
                writer.writerow(['model_name', 'param', 'gpu', 'FLOPS', 'class_num', 't_img_num', 't_instance_num', 'v_img_num', 
                                 'v_instance_num', 'imgsz','batch', 'epoch', 'preprocess_time', 'train_time', 'val_time', 'save_model_time', 
                                 'scheduler_time', 'epoch_time', 'elapsed', 'remaining', 'gpu_usage', 'cpu_usage'])

            writer.writerow([model_name, param, gpu, FLOPS, class_num, t_img_num, t_instance_num, v_img_num, v_instance_num,
                            imgsz, batch, epoch, preprocess_time, train_time, val_time, save_model_time, scheduler_time, epoch_time, 
                            elapsed, remaining, gpu_usage, cpu_usage])
    ...
    def format_meter(n, total, elapsed, rate=None, initial=0, *args, **kwargs):
        ...
        gpu_usage = sum(Astrago.gpu_memory_usage) / (len(Astrago.gpu_memory_usage)-1) if n != 0 else sum(Astrago.gpu_memory_usage) / len(Astrago.gpu_memory_usage)
        cpu_usage = sum(Astrago.cpu_usage) / (len(Astrago.cpu_usage)-1) if n != 0 else sum(Astrago.cpu_usage) / len(Astrago.cpu_usage)
        ...
        if n == total:
            if not hasattr(Astrago, 'last_epoch_done') or not Astrago.last_epoch_done:  # 마지막 에폭이 한 번만 출력되도록 확인하는 변수
                Astrago.save_metrics_to_csv(Astrago.model_name, Astrago.param, Astrago.gpu, Astrago.FLOPS, class_num, t_img_num, 
                                            t_instance_num, v_img_num, v_instance_num, imgsz, batch, n, preprocess_time, train_time, 
                                            val_time, savemodel_time, scheduler_time, epoch_per_time, elapsed, t, gpu_usage, cpu_usage)
...
class Inference(tqdm):
    ...
    csv_file_path = "/DATA/predict_yolov5s.csv"
    model_name = 'yolov5s'
    param = 0
    gpu = ''
    FLOPS = 14
